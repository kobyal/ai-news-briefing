...
def build_and_save_html(topic: str = "AI", tool_context=None) -> dict:
    ...
    json_path = os.path.join(out_dir, f"briefing_{ts}.json")
    with open(json_path, "w", encoding="utf-8") as f:
        _json.dump({"source": "adk", "briefing": data, "briefing_he": he}, f, ensure_ascii=False)
    print(f"Publisher -- saved to {path}")
    return {"saved_to": path, "json_saved_to": json_path, "success": True}
