    ...
    async function load() {
      ...
      try {
        ...
        if (indexed?.date) {
          ...
          // Pre-2026-05-18 JSONs lack story_id so the lookup above returns nothing.
          // The search index already has the headline/summary/og_image we need to
          // render a readable page, so use it as a minimal story fallback.
          if (indexed.headline) {
            setStory(indexed as unknown as NewsItem);
