...
3. פעלים ישראליים חיים: "שוברת", "בולעת", "כובשת", "מסתבכת", "זורקת", "הולכת ל"
   ⚠ אבל: פעלים של השלמה ("בולעת", "רוכשת", "עוברת ל", "קונה") מותרים רק כשהעסקה
   נחתמה או הוכרזה רשמית. אם המקור אומר "in talks" / "reportedly" / "nearing a deal"
   — הכותרת העברית חייבת לשקף זאת: "במגעים לרכוש", "מנהלת מו״מ", "על הכוונת",
   "בדרך ל". כותרת שמכריזה על רכישה שלא נחתמה היא שגיאה עובדתית, לא סגנון.
   ❌ "NVIDIA בולעת את Hugging Face" (כשהמקור: "reportedly in talks to be acquired")
   ✅ "NVIDIA במגעים לרכוש את Hugging Face: בית המודלים הפתוחים על הכוונת"
   אותו כלל חל על summaries_he ו-details_he — אל תתאר עסקה לא חתומה כעובדה מוגמרת.
...
- tldr_he: תרגום של tldr — בדיוק אותו מספר בולטים כמו ב-tldr, אחד לאחד ובאותו סדר.
  אל תוסיף בולט סיכום משלך ואל תאחד שני בולטים לאחד: כל בולט מקושר לכתבה לפי המיקום
  שלו ברשימה, ובולט עודף מוביל לקישור שבור.
