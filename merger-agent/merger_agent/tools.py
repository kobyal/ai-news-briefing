...
_SHARE_ICONS = {
    "x":        '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.259 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',
    ...
    "copy":     '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>',
}


def _share_bar_html(share_url: str) -> str:
    """Build social share button bar for a news card."""
    if not share_url:
        return ""
    btns = "".join(
        f'<button class="share-btn" title="Share on {lbl}" onclick="shareCard(this,\'{key}\')">{_SHARE_ICONS[key]}</button>'
        for key, lbl in [("x", "X"), ("linkedin", "LinkedIn"), ("facebook", "Facebook"),
                          ("whatsapp", "WhatsApp"), ("telegram", "Telegram")]
    )
    copy_btn = f'<button class="share-btn" title="Copy link" onclick="shareCard(this,\'copy\')">{_SHARE_ICONS["copy"]}</button>'
    return f'<div class="share-bar"><span class="share-label">Share</span>{btns}{copy_btn}</div>'
...
{xai_trending_html}
    ...
    for idx, item in enumerate(news_items):
        ...
        share_bar = _share_bar_html(share_url)

        cards += f"""<div class="news-card" data-vendor="{_esc(vendor.lower())}" data-share-url="{_esc(share_url)}" data-share-title="{_esc(headline[:120])}">
...
/* Social share */
.share-bar{{display:flex;gap:5px;margin-top:10px;padding-top:8px;border-top:1px solid #f1f5f9;align-items:center;flex-wrap:wrap}}
.share-btn{{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:6px;border:none;cursor:pointer;background:rgba(15,23,42,.06);color:#64748b;transition:background .15s,color .15s;flex-shrink:0;padding:0;line-height:1}}
.share-btn:hover{{background:#1c1917;color:#fcd34d}}
.share-btn.copied{{background:#16a34a!important;color:#fff!important}}
.share-label{{font-size:11px;color:#94a3b8;margin-right:2px;user-select:none}}
</style></head><body>
<div class="header">
